  ...
  methods: {
    async getGoals() {
      try {
        ...
        this.temporaryGoals = response.data;
        for(let i = 0; i < this.temporaryGoals.length; i++) {
          if(this.temporaryGoals[i].page === this.page) {
            this.goals.push(this.temporaryGoals.splice(i,1)[0]);
          }
        }
        return true;
      } catch (error) {
